...
async function createServer() {
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      if (!GOOGLE_API_KEY) {
        return res.status(500).json({ error: { code: 'SERVER_MISCONFIG', message: 'Server misconfigured: GOOGLE_API_KEY not set.' } });
      }
      ...
      // Guard the model call with a timeout (e.g., 25 seconds)
      ...
      if (err?.code === 'TIMEOUT') {
        return res.status(503).json({ error: { code: 'TIMEOUT', message: 'Model generation timed out. Please try again.' } });
      }
      res.status(500).json({ error: { code: 'GENERATION_FAILED', message: err?.message || 'Failed to generate response' } });
    }
  });
...
// Note: This should be the last middleware; since we start the server inside createServer,
// this is here for safety in case of any unhandled errors bubbling up.
