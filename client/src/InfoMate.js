...
import { COURSES } from './data/staticCourses';
import { 
  Mic, 
  MicOff, 
  Send, 
...
const InfoMate = () => {
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      ...
      const supported = speechService.initializeSpeechRecognition();
  ...
  const handleSendMessage = async (messageText = inputText) => {
    ...
    try {
      ...
      console.error('Chat error:', err); // Full error for debugging
      setIsLoading(false);
      const botMessage = {
        id: Date.now() + 1,
        text: err?.userMessage || 'Sorry, I ran into a problem answering that. Please try again shortly.',
        sender: 'bot',
        timestamp: new Date()
      };
      setMessages(prev => [...prev, botMessage]);
  ...
  const handleNewChat = () => {
    ...
    const welcomeMessage = {
      id: Date.now(),
      text: `👋 Hello! I'm InfoMate, your intelligent ICT Department assistant at Marwadi University.\n\nI can help you with:\n• 👨‍🏫 Faculty information and contact details\n• 📚 Course details and curriculum\n• 🎓 Admission procedures\n\n${!speechSupported ? '⚠️ Note: Speech recognition is not supported in your browser.' : '🎤 You can type or speak to me!'}\n\nHow can I assist you today?`,
      sender: 'bot',
      timestamp: new Date()
    };
